...
static void lv_event_value_update(lv_event_t *e)
{
    ...
    lv_obj_t *sub_obj = ui_comp_get_child(obj, BLE_SW_ENABLE);
    if (sub_obj)
    {
        if (BLESetStep)
        {
...
static void btn_event_clicked(lv_event_t *e)
{
    ...
    lv_obj_t *obj = lv_event_get_target(e);
    enum UI_COMP_BLE comp_id = (enum UI_COMP_BLE)((uint32_t)lv_event_get_user_data(e));
...
lv_obj_t *ui_comp_ble_set_create(lv_obj_t *parent)
{
    ...
    lv_obj_add_event_cb(btn_sw, child_event_value_changed, LV_EVENT_VALUE_CHANGED, (void *)BLE_SW_ENABLE);
    ...
    lv_obj_align(label, LV_ALIGN_CENTER, 0, 0);
    lv_obj_add_event_cb(btn_getName, btn_event_clicked, LV_EVENT_CLICKED, (void *)BLE_BTN_GET_NAME);
    ...
    lv_obj_add_event_cb(btn_sw_sendMsg, child_event_value_changed, LV_EVENT_VALUE_CHANGED, (void *)BLE_SW_SEND_MSG);
    ...
    lv_obj_align(label, LV_ALIGN_CENTER, 0, 0);
    lv_obj_add_event_cb(btn_return, btn_event_clicked, LV_EVENT_CLICKED, (void *)BLE_BTN_EXIT);

    lv_obj_t **children = lv_mem_alloc(sizeof(lv_obj_t *) * BLE_NUM);
    lv_memset_00(children, sizeof(lv_obj_t *) * BLE_NUM);
    children[BLE_SW_ENABLE] = btn_sw;
    children[BLE_LABEL_MSG] = label_msg;
