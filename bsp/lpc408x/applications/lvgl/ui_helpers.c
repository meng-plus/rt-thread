...
void _lv_event_child_notify(lv_obj_t *target, uint32_t event_id, void *param)
{
   ...
   if (target && target->spec_attr->children != NULL)
   {
      for (size_t i = 0; i < target->spec_attr->child_cnt; i++)
      {
         lv_obj_t *child_obj = target->spec_attr->children[i];
         lv_event_send(child_obj, event_id, param);
      }
   }
}
